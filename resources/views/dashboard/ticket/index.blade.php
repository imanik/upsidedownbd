...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow sm:rounded-lg">
        <div class="flex items-center justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{  ($filter['customer']? $filter['customer']->name . '\'s' : '') . __('Tickets') }}</p>

            <div class="flex gap-4">
                {{-- @if(Auth::user()->is_admin)
                <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">Incomes: {{ __($incomes) }} ৳</p>
                @endif --}}
                ...
                <form class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                    ...
                    @endif
                    <div class="text-sm text-gray-700">
                        <input type="date" class="rounded border-gray-400 w-full h-full lg:w-32 xl:w-40 appearance-none text-sm text-gray-700" name="start_date" id="start_date" value="{{ $filter['start_date']? date('Y-m-d', strtotime($filter['start_date'])) : null }}" onchange="this.form.submit()">
                    </div>
                    <div class="text-sm text-gray-700">
                        <input type="date" class="rounded border-gray-400 w-full h-full lg:w-32 xl:w-40 appearance-none text-sm text-gray-700" name="end_date" id="end_date" value="{{ $filter['end_date']? date('Y-m-d', strtotime($filter['end_date'])) : null }}" onchange="this.form.submit()">
                    </div>
        ...
        <table class="min-w-full w-full align-middle overflow-hidden" id="table">
            <thead>
                <tr>
                    ...
                    <th class="w-28 px-6 py-3 border-b border-gray-200 bg-gray-100 text-left text-xs leading-4 font-semibold text-gray-500 uppercase tracking-wider">{{ __('Branch') }}</th>
                    <th class="w-36 px-6 py-3 border-b border-gray-200 bg-gray-100 text-left text-xs leading-4 font-semibold text-gray-500 uppercase tracking-wider">{{ __('Customer') }}</th>
            ...
            <tbody class="bg-white">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 font-semibold text-gray-700">{{ optional($ticket->customer)->name }}</div>
                        <div class="text-sm leading-5 font-semibold text-gray-700">[{{ optional($ticket->customer)->mobile }}]</div>
                        <div class="text-sm leading-5 text-gray-600">{{ optional($ticket->customer)->address }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        @php
                        $classes = [
                    ...
                    <td class="px-6 py-4 whitespace-wrap text-right border-b border-gray-200 text-sm leading-5 font-semibold">
                        ...
                        <div class="flex justify-end">
                            ...
                            @if($ticket->payment_status != "paid" && empty(Auth::user()->is_admin) && empty(Auth::user()->role))
                            <a href="{{ route('ticket.detail', $ticket->reference) }}" class="text-indigo-600 hover:text-indigo-900 ml-3" title="show">
                                Pay
                            </a>
                            @endif

                            <a href="{{ $ticket->photos_link ?? '#' }}" class="text-indigo-600 hover:text-indigo-900 ml-3" title="@if(empty($ticket->photos_link)) photos not uploaded yet @else photo link @endif">
                            ...
                            <a href="{{ route('ticket.show', $ticket->id) }}" class="text-indigo-600 hover:text-indigo-900 ml-3" title="show">
                            ...
                            <a href="{{ route('ticket.ticket_edit', $ticket->id) }}" class="text-indigo-600 hover:text-indigo-900 ml-3" title="edit">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            @endif

                            @if(Auth::user()->is_admin)
                            <a href="#" class="text-red-600 hover:text-red-900 ml-3" id="data-route-{{ $ticket->id }}" data-route="{{ route('ticket.destroy', [$ticket->id]) }}" onclick="event.preventDefault();if(confirm('Do you really want to delete this ticket')){document.getElementById('delete-form').setAttribute('action', document.querySelector('#data-route-'+{{ $ticket->id }}).dataset.route); document.getElementById('delete-form').submit();}">
