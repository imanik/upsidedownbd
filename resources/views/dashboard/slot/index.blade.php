...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow sm:rounded-lg">
        <div class="flex items-center justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Slots') }}</p>

            <form class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                @if(Auth::user()->is_admin)
                <div class="text-sm text-gray-700">
                    <select class="rounded border-gray-400 w-full h-full lg:w-32 xl:w-40 appearance-none text-sm text-gray-700" name="branch" id="banch" onchange="this.form.submit()">
        ...
        <table class="min-w-full w-full align-middle overflow-hidden" id="table">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 border-b border-gray-200 bg-gray-100 text-left text-xs leading-4 font-semibold text-gray-500 uppercase tracking-wider">{{ __('Time') }}</th>
            ...
            <tbody class="bg-white">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 font-semibold text-gray-700">{{ $slot->name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-600">{{ $slot->time }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-600">{{ $slot->branch->name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap text-right border-b border-gray-200 text-sm leading-5 font-semibold">
                        <div class="flex justify-end">
                            <a href="{{ route('slot.edit', $slot->id) }}" class="text-indigo-600 hover:text-indigo-900 ml-3" title="edit">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <a href="#" class="text-red-600 hover:text-red-900 ml-3" title="delete" id="data-route-{{ $slot->id }}" data-route="{{ route("slot.destroy", [$slot->id]) }}" onclick="event.preventDefault();if(confirm('Do you really want to delete this slot')){document.getElementById('delete-form').setAttribute('action', document.querySelector('#data-route-'+{{ $slot->id }}).dataset.route); document.getElementById('delete-form').submit();}">
