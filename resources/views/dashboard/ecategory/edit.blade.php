...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <form action="{{ route('ecategory.update',[$ecategory->id]) }}" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="type" value="{{ $ecategory->type }}" id="type">
            @csrf
            @method('PATCH')
            <dl x-data="{type: '{{ $ecategory->type }}'}" class="py-6">
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('name') border-red-400 @enderror" type="text" maxlength="191" name="name" id="name" autocomplete="off" placeholder="{{ __('Category name') }}" value="{{ old('name') ?? $ecategory->name }}">
                        @error('name')
