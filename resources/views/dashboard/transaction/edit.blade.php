                ...
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('transaction.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">{{ __('Categories') }}</a>
                        </div>
                    </li>
...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Category Edit') }}</p>
        </div>
        <form action="{{ route('transaction.update',[$transaction->id]) }}" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="type" value="{{ $transaction->type }}" id="type">
            @csrf
            @method('PATCH')
            <dl x-data="{type: '{{ $transaction->type }}'}" x-init=" getAccountCategory();" class="py-6">
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <span class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded"  name="date" id="date" min="{{ Carbon\Carbon::now()->format('Y-m-d') }}" max="{{ Carbon\Carbon::now()->addMonth(1)->format('Y-m-d') }}"  value="{{ old('date') ?? $transaction->date }}">{{ $transaction->getDate();}}</span> <span></span>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="relative">
                            <select class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('account_category') border-red-400 @enderror" name='account_category' id="account_category">
                                <option value="">{{ __('Select Category') }}</option>
                                @foreach ($account_categories as $account_category)
                                <option value="{{ $account_category->id }}" @if($transaction->accountCategory && $transaction->accountCategory->id == $account_category->id) selected @endif @if($account_category->id == old('account_category')) selected @endif>{{ __(App\Helpers::asText($account_category->name)) }}</option>
                                @endforeach
                            </select>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('amount') border-red-400 @enderror" type="text" name="amount" id="amount"  autocomplete="off" placeholder="{{ __('Amount') }}" value="{{ old('amount')?? $transaction->amount }}">
                        @error('amount')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2">
                        <textarea class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('notes') border-red-400 @enderror" type="text" name="notes" id="notes"  rows="3" placeholder="{{ __('Notes') }}">{{ old('notes')?? $transaction->description }}</textarea>
                        @error('notes')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
