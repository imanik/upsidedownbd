...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                ...
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('fmenu.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">{{ __('Bundles') }}</a>
                        </div>
                    </li>
        ...
        <div class="mt-6 flex space-x-3 md:mt-0 md:ml-4">
            @if(Helpers::isRouteValid('fmenu.create'))
            <a href="{{ route('fmenu.create') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500">
...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Menu Edit') }}</p>
        </div>
        <form action="{{ route('fmenu.update',[$fmenu->id]) }}" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="type" value="{{ $fmenu->type }}" id="type">
            @csrf
            @method('PATCH')
            <dl x-data="{type: '{{ $fmenu->type }}'}" class="py-6">
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input type="text" class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('title') border-red-400 @enderror" name="title" id="title" placeholder="{{ __('Enter Title') }}" value="{{ old('title')?? $fmenu->title }}">
                        @error('title')
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input type="text" class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('subtitle') border-red-400 @enderror" name="subtitle" id="subtitle" placeholder="{{ __('Enter Sub Title') }}" value="{{ old('subtitle')?? $fmenu->subtitle }}">
                        @error('subtitle')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                ...
                <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1 relative w-64">
                        @php
                            ...
                            $photo = !empty($fmenu->photo) && Storage::disk('public')->exists($fmenu->photo) ? Storage::disk('public')->url($fmenu->photo) : $dummy_photo;
                        @endphp
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="relative">
                            <select class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('branch') border-red-400 @enderror" name='branch' x-on:change="branch_id = $event.target.value; getBranch(); ">
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="mt-1 flex rounded shadow-sm">
                            <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('normal_price') border-red-400 @enderror" type="text" oninput="this.value = this.value.replace(/[^0-9]/g, '');" name="normal_price" id="normal_price" autocomplete="off"  placeholder="{{ __('Normal Price') }}" value="{{ old('normal_price')?? $fmenu->normal_price }}">
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="mt-1 flex rounded shadow-sm">
                            <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('offer_price') border-red-400 @enderror" type="text" oninput="this.value = this.value.replace(/[^0-9]/g, '');" name="offer_price" id="offer_price" autocomplete="off"  placeholder="{{ __('Offer Price') }}" value="{{ old('offer_price')?? $fmenu->offer_price }}">
                            <span class="inline-flex items-center px-6 rounded-r border border-l-0 border-gray-300 bg-gray-50 text-gray-500 sm:text-sm">&#2547;</span>
                        </div>
                        @error('offer_price')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2">
                        <textarea class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('description') border-red-400 @enderror" name="description" rows="4" placeholder="{{ __('Enter Description') }}">{{ old('description')?? $fmenu->description }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
...
<script>
    function init() {
        return {
            no_of_adult: @json($fmenu->regular_ticket_count),
            no_of_child: @json($fmenu->child_ticket_count),
            offer_price: @json($fmenu->offer_price),
            branch_id: @json(old('branch')?? $fmenu->branch_id),
            branch: null,
