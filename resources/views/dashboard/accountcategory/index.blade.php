...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="mb-5 bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Categories') }}</p>
        </div>
            ...
            <tbody class="bg-white">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 font-semibold text-gray-700">{{ $accountcategory->name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 font-semibold text-gray-700">{{ $accountcategory->type }}</div>
                    </td>

                    <td class="px-6 py-4 whitespace-wrap text-right border-b border-gray-200 text-sm leading-5 font-semibold">
                        <div class="flex justify-end">
                            <a href="{{ route('accountcategory.edit', $accountcategory->id) }}" class="text-indigo-600 hover:text-indigo-900 mx-3" name="edit">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </a>
                            <a href="#" class="text-red-600 hover:text-red-900" id="data-route-{{ $accountcategory->id }}" name="delete" data-route="{{ route("accountcategory.destroy", [$accountcategory->id]) }}" onclick="event.preventDefault();if(confirm('Do you really want to delete this category')){document.getElementById('delete-form').setAttribute('action', document.querySelector('#data-route-'+{{ $accountcategory->id }}).dataset.route); document.getElementById('delete-form').submit();}">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
