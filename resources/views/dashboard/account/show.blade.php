            ...
            <dl x-data="{type: ''}" class="py-6">
                ...
                <div class="w-half px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dt class="text-sm leading-5 font-semibold text-gray-700 tracking-wider sm:col-span-1 sm:text-right pr-8">
                        ...
                        <p>{{ optional($income_detail->accountCategory)->name}}</p>
                        @endforeach
                        <br>
                    ...
                    <dd class="text-sm leading-5 font-semibold text-gray-700 tracking-wider sm:col-span-1 sm:text-right pr-8">
                        <p>{{ ($income->total - $income->cc_sale) }}</p>
                        <p>{{ (round($income->cc_sale)) }}</p>
                        @foreach ($incomes_detail as $income_detail)
                        <p>{{ (round($income_detail->amount))  }}</p>
                        @endforeach
                        <br>
                        <p class="text-blue-800 font-extrabold">{{ (round($income->total)+ App\Helpers::getIncomeSumByDate($income)) }}</p>
                    </dd>
                </div>
                ...
                <div class="w-half px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dt class="text-sm leading-5 font-semibold text-gray-700 tracking-wider sm:col-span-1 sm:text-right pr-8">
                        ...
                        <p>{{ optional($expense_detail->accountCategory)->name}}</p>
                        @endforeach
                        <br>
                    ...
                    <dd class="text-sm leading-5 font-semibold text-gray-700 tracking-wider sm:col-span-1 sm:text-right pr-8">
                        ...
                        @foreach ($expenses_detail as $expense_detail)
                        <p>{{ (round($expense_detail->amount))  }}</p>
                        @endforeach
                        <br>
                        <p class="text-red-700 font-extrabold">{{  App\Helpers::getIncomeSumByDate($income->date) }}</p>
