        ...
        <table class="min-w-full w-full align-middle overflow-hidden" id="table">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 border-b border-gray-200 bg-gray-100 text-left text-xs leading-4 font-semibold text-gray-500 uppercase tracking-wider">{{ __('Description') }}</th>
            ...
            <tbody class="bg-white">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-700">{{ $bundle->title }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-700">{{ $bundle->subtitle }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-700">{{ optional($bundle->branch)->name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="flex items-center">
                            <div class="ml-4">
                                ...
                                <span class="text-sm leading-5 font-semibold text-gray-700">Facility: </span>{{ optional($bundle->facility)->title }}
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="flex items-center">
                            <div class="ml-4">
                                <span class="text-sm leading-5 font-semibold text-gray-600">Regular Price: </span>{{ $bundle->normal_price }}<br>
                                <span class="text-sm leading-5 font-semibold text-gray-600">Offer Price: </span>{{ $bundle->offer_price }}
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-700">{{ $bundle->description }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap text-right border-b border-gray-200 text-sm leading-5 font-semibold">
                        <div class="flex justify-end">
                            <a href="{{ route('bundle.edit', $bundle->id) }}" class="text-indigo-600 hover:text-indigo-900 ml-3" title="edit">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <a href="#" class="text-red-600 hover:text-red-900 ml-3" title="delete" id="data-route-{{ $bundle->id }}" data-route="{{ route('bundle.destroy', [$bundle->id]) }}" onclick="event.preventDefault();if(confirm('Do you really want to delete this bundle')){document.getElementById('delete-form').setAttribute('action', document.querySelector('#data-route-'+{{ $bundle->id }}).dataset.route); document.getElementById('delete-form').submit();}">
