...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <section class="text-gray-600 body-font">
        <form class="container mx-auto flex flex-wrap" action="{{ route('payment.update',[$payment->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div x-data="init()" class="lg:w-2/3 mx-auto">
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2">
                        <div class="flex flex-col flex-wrap mt-2 h-96">
                            ...
                            <label class="w-1/3 mb-4 inline-flex items-center cursor-pointer">
                                <input type="radio" class="focus:ring-blue-400 focus:border-blue-400 block sm:text-sm border-gray-400 w-5 h-5 border text-blue-700 cursor-pointer" name="slot" x-on:change="data.slot = $event.target.dataset.name" value="{{ $slot->id }}" @if($slot->id == $payment->slot_id) checked @endif data-name="{{ $slot->name }}" {{ old('status') == $slot->id ? "checked" : "" }}>
                                <span class="ml-2"> {{ $slot->name }} </span>
                            </label>
                            @endforeach
                        </div>
                        @error('slot')
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('child_number') border-red-400 @enderror" type="number" oninput="this.value = this.value.replace(/[^0-9]/g, '');" x-on:change="data.no_of_child = $event.target.value" x-on:keyup="data.no_of_child = $event.target.value" min="0" max="100" name="child_number" id="child_number" autocomplete="off" placeholder="{{ __('EX. 3') }}" value="{{ old('child_number') ?? $payment->child_payment_count }}">
                        @error('child_number')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                        <div class="relative">
                            <select class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded" name="volunteer">
                                ...
                                <option value="{{ $volunteer->id }}" @if($payment->volunteer_id == $volunteer->id) selected @endif>{{ __(App\Helpers::asText($volunteer->name)) }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('volunteer')
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                        <div class="relative">
                            <select class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded" name="facility_provider">
                                ...
                                <option value="{{ $facility_provider->id }}" @if($payment->facility_provider_id == $facility_provider->id) selected @endif>{{ __(App\Helpers::asText($facility_provider->name)) }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="flex flex-col mt-2">
                            ...
                            @endphp
                            @foreach (config('upsidedown.payment.statuses') as $status)
                            <label class="mb-4 inline-flex items-center cursor-pointer">
                                <input type="radio" class="block sm:text-sm border-gray-400 w-5 h-5 border cursor-pointer {{ $classes[$status]}}" name="status" value="{{ $status }}" @if($status==$payment->status) checked @endif {{ old('status') == $status ? "checked" : "" }}>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('photos_link') border-red-400 @enderror" type="text" maxlength="191" name="photos_link" id="photos_link" autocomplete="off" placeholder="{{ __('Photos Link') }}" value="{{ old('photos_link')?? $payment->photos_link }}">
                        @error('photos_link')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                    ...
                    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div x-show="preview"
                            ...
                            <div class="bg-white px-4 py-5 sm:px-6 border-b border-gray-200">
                                ...
                                <p class="mt-1 max-w-2xl text-sm text-gray-500" id="payment_details"></p>
                            </div>
    ...
    function init() {
        return {
            ...
            cost: {
                ...
                facility: payment.facility.cost,
            },
            data: {
                visit_date: payment.date.split(" ")[0],
                slot: payment.slot.name,
                no_of_adult: payment.regular_payment_count,
                no_of_child: payment.child_payment_count,
                facility: payment.facility.cost,
                facility_text: payment.facility.title,
                branch: branch.name,
            },
            user: {
