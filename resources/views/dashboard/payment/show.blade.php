        ...
        <form class="container mx-auto flex flex-wrap" action="{{ route('pay') }}" method="Post">
            ...
            <div class="lg:w-2/3 mx-auto">
                ...
                <div>
                                            ...
                                            <dl>
                                                ...
                                                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 flex flex-col">
                                                        <span class="text-base font-semibold"><span>{{ $payment->branch->name }}</span></span>
                                                        @foreach($payment->slots as $payment_slot)
                                                        <span class="text-sm font-semibold">
                                                            Slot: <span class="font-normal">{{ $payment_slot->slot->name }}</span>
                                                        </span>
                                                        @endforeach
                                                    </dd>
                                                </div>
                                                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 flex flex-col">
                                                        <span class="text-base font-semibold">{{ $payment->customer->name }}</span>
                                                        <span class="text-sm">{{ $payment->customer->mobile }}</span>
                                                        <span class="text-sm">{{ $payment->customer->email }}</span>
                                                        <span class="text-sm">{{ $payment->customer->address }}</span>
                                                    </dd>
                                                </div>
                                                <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    <dt class="text-sm font-medium text-gray-500">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 flex flex-col">
                                                        <span class="text-sm font-semibold" x-show="data.no_of_adult > 0">Adult: <span class="font-normal">{{ $payment->regular_payment_count }}</span></span>
                                                        <span class="text-sm font-semibold" x-show="data.no_of_child > 0">Child: <span class="font-normal">{{ $payment->child_payment_count }}</span></span>
                                                ...
                                                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                                        <span>{{ $payment->grand_total }}</span> ৳
                                                        <span class="text-sm">{{ App\Helpers::asText($payment->status) }}</span>
                                                    </dd>
                                                </div>
                                            </dl>
                                        </div>
                    ...
                    @if (!$payment->payment || !in_array($payment->payment->status, ['Processing', 'Complete']))
