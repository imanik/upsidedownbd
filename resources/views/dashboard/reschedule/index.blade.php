                ...
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('reschedule.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">{{ __('Reschedules') }}</a>
                        </div>
                    </li>
...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="mb-5 bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Reschedules') }}</p>
        </div>
            ...
            <tbody class="bg-white">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="text-sm leading-5 text-gray-600">{{ $reschedule->ticket->reference }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap border-b border-gray-200">
                        <div class="flex items-center">
                            <div class="ml-4">
                                <div class="text-sm leading-5 font-semibold text-gray-700">{{ $reschedule->ticket->customer->name . ' [' . $reschedule->ticket->customer->mobile . ']' }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-wrap text-right border-b border-gray-200 text-sm leading-5 font-semibold">
                        <div class="flex justify-end">
                            <a href="#" class="text-red-600 hover:text-red-900 ml-3" title="delete" id="data-route-{{ $reschedule->id }}" data-route="{{ route('reschedule.destroy', [$reschedule->id]) }}" onclick="event.preventDefault();if(confirm('Do you really want to delete this reschedule')){document.getElementById('delete-form').setAttribute('action', document.querySelector('#data-route-'+{{ $reschedule->id }}).dataset.route); document.getElementById('delete-form').submit();}">
