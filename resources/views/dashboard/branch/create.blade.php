...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow-md overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Branch Create') }}</p>
        </div>
            ...
            <dl class="py-6">
                ...
                <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1 relative h-56 rounded">
                        <div class="flex text-sm text-gray-700 border-2 border-gray-400 border-dashed rounded-md h-56 w-full bg-gray-50 cursor-pointer" onclick="this.querySelector('.form-file').click()">
                            <img class="object-fill w-full rounded-md" src="https://via.placeholder.com/400x500.png/ddd/333" id="photo_holder" title="Click to upload photo">
                            <input type="file" class="form-file hidden" name="photo" id="photo" onchange="document.getElementById('photo_holder').src = window.URL.createObjectURL(this.files[0]);" accept=".png, .jpg, .jpeg">
                        </div>
                        <div class="absolute bottom-0 right-0">
                            <svg class="w-5 h-5 cursor-pointer text-blue-300" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" onclick="if(confirm('Do you want to refresh this photo??')){document.getElementById('photo_holder').src = '{{ 'https://via.placeholder.com/400x500.png/ddd/333' }}';document.getElementById('photo').value = null;}"><path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" /></svg>
                        </div>
                        @error('photo')
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('name') border-red-400 @enderror" type="text" maxlength="191" name="name" id="name" autocomplete="off" placeholder="{{ __('Branch name') }}" value="{{ old('name') }}">
                        @error('name')
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('address') border-red-400 @enderror" type="text" maxlength="400" name="address" id="address" autocomplete="off" placeholder="{{ __('Branch address') }}" value="{{ old('address') }}">
                        @error('address')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-2">
                        <div class="mt-2">
                            ...
                            <label class="w-auto pr-4 mr-4 inline-flex items-center cursor-pointer">
                                <input type="radio" class="focus:ring-blue-400 focus:border-blue-400 block sm:text-sm border-gray-300 w-5 h-5 border text-blue-700 cursor-pointer" name="status" value="{{ App\Helpers::asValue($status) }}" {{ old('status') == App\Helpers::asValue($status) ? "checked" : "" }}>
