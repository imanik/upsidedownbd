...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Coupon Create') }}</p>
        </div>
        <form action="{{ route('coupon.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <dl class="py-6" aria-label="init">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="relative">
                            <select class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('branch_id') border-red-400 @enderror" name='branch_id' id='branch_id'>
                                ...
                                <option value="{{ $branch->id }}" @if(old('branch_id')==$branch->id) selected @endif>{{ __(App\Helpers::asText($branch->name)) }}</option>
                                @endforeach
                            </select>
                            @error('branch_id')
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('code') border-red-400 @enderror" type="text" maxlength="191" name='code' id='code' autocomplete="off" placeholder="{{ __('Coupon code') }}" value="{{ old('code') }}">
                        @error('code')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-6">
                    ...
                    <dd class="px-4 mt-2 text-sm text-gray-700">
                        <div class="flex items-center">
                            <input id="days_{{$indexs[$key]}}" name="days[{{$indexs[$key]}}]" type="checkbox" class="form-checkbox w-5 h-5 text-indigo-600 transition duration-150 ease-in-out rounded cursor-pointer" @if(!old('days')) checked @endif @if(isset($days) && isset($days[$indexs[$key]]) && $days[$indexs[$key]]=="on" ) checked @endif>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('start_date') border-red-400 @enderror" type="date" name="start_date" id="start_date" value="{{ old('start_date') }}">
                        @error('start_date')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('end_date') border-red-400 @enderror" type="date" name="end_date" id="end_date" value="{{ old('end_date') }}">
                        @error('end_date')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
