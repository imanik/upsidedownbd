...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
        ...
        <div class="mt-6 flex space-x-3 md:mt-0 md:ml-4">
            ...
            <a href="{{ route('attendance.create') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500">
...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Attendance Edit') }}</p>
        </div>

        <form class="" action="{{ route('attendance.update', $attendance->id) }}" method="POST">
            @csrf
            @method('patch')
            <dl class="py-6">
                <div class="px-4 py-3 sm:grid sm:grid-cols-4 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('date') border-red-400 @enderror" type="date" name="date" value="{{ old('date')?? date('Y-m-d', strtotime($attendance->date)) }}">
                        @error('date')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </dd>
                ...
                <div class="px-4 py-3 sm:grid sm:grid-cols-4 sm:gap-4 sm:px-6">
                    <dt class="text-sm leading-10 font-semibold text-gray-700 tracking-wider text-right pr-8">
                        {{ __('Check In') }} <span class="text-red-500">*</span>
                    </dt>
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('check_in') border-red-400 @enderror" type="time" name="check_in" value="{{ old('check_in')?? (date('H:i', strtotime($attendance->check_in))) }}">
                        @error('check_in')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </dd>
                    ...
                    <dt class="text-sm leading-10 font-semibold text-gray-700 tracking-wider text-right pr-8">
                        {{ __('Check Out') }} <span class="text-red-500">*</span>
                    </dt>
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('check_out') border-red-400 @enderror" type="time" name="check_out" value="{{ old('check_out')?? ($attendance->check_out? date('H:i', strtotime($attendance->check_out)) : date('H:i')) }}">
                        @error('check_out')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </dd>
