...
<div class="px-4 sm:px-6 lg:max-w-7xl lg:mx-auto lg:px-8">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <p class="max-w-2xl text-md leading-10 text-gray-700 text-2xl font-semibold">{{ __('Facility Edit') }}</p>
        </div>
        <form action="{{ route('facility.update',[$facility->id]) }}" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="type" value="{{ $facility->type }}" id="type">
            @csrf
            @method('PATCH')
            <dl class="py-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="relative">
                            <select class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('branch_id') border-red-400 @enderror" name='branch_id' id='branch_id'>
                                ...
                                <option value="{{ $item->id }}" @if ($item->id == old('branch_id') or $item->id == $facility->branch_id) selected @endif>{{ __(App\Helpers::asText($item->name)) }}</option>
                                @endforeach
                            </select>
                            @error('branch_id')
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 sm:text-sm border-gray-300 rounded @error('title') border-red-400 @enderror" type="text" maxlength="191" name='title' id='title' autocomplete="off" placeholder="{{ __('Facility name') }}" value="{{ old('title')?? $facility->title }}">
                        @error('title')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </dd>
                ...
                <div class="w-full px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                        <div class="mt-1 flex rounded shadow-sm">
                            <input class="focus:ring-blue-400 focus:border-blue-400 block w-full px-4 z-10 sm:text-sm border-gray-300 rounded-l @error('cost') border-red-400 @enderror" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" maxlength="20" name="cost" id="cost" autocomplete="off" placeholder="{{ __('Cost') }}" value="{{ old('cost')?? $facility->cost }}">
